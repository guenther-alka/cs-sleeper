...
func usage() {
	fmt.Println(`cs-sleeper -- spin down idle ZFS pool disks (v1.0)
...
Commands:
  ...
  status      one-shot report of managed disks and their state
...
Config:
  ...
  hd         = sda,ada0   # disks to manage (device names, see README matrix)
  exclude    =            # disks to never touch
  pools      =            # pools considered for import/export safety
